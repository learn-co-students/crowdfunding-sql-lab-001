...
describe "crowdfunder sql" do
  ...
  describe "questions" do
    ...
    it 'selects the titles of all projects that have met their funding goal' do
      expect(@db.execute("SELECT projects.title, (projects.funding_goal - SUM(pledges.amount)) AS amount_left FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING amount_left <= 0;")).to eq([["My book on SQL", 0], ["The next Inna-Gadda-Davida", -142]])
    end
    ...
    it 'selects the category names, and pledge amounts of all pledges in the music category' do
      expect(@db.execute("SELECT projects.category, pledges.amount FROM projects JOIN pledges ON projects.id = pledges.project_id WHERE projects.category = 'music';")).to eq([["music", 40], ["music", 24], ["music", 34], ["music", 12], ["music", 40], ["music", 40], ["music", 20], ["music", 230]])
    end

    it 'selects the category names and the sum total of the pledge amounts of all the pledges in the book category' do
      expect(@db.execute("SELECT projects.category, SUM(pledges.amount) FROM projects JOIN pledges ON projects.id = pledges.project_id WHERE projects.category = 'books' GROUP BY projects.category;")).to eq([["books", 140]])
    end
  end
end
